...
public class Upickup : MonoBehaviour {
	...
	int RandomM = 0;       
	string RandomStrInt = "";        
	string RandomStr = "";         // Use this for initialization        
	...
	public void Pickup(){                                
		...
		for (int i = 0; i < cells.Length; i++) {                      
			if (cells [i].transform.childCount > 0) {//判断当前格子是否有物体                                //如果有，并且一样的                               
				if (Imagesingle.overrideSprite.name == cells [i].transform.GetChild (0).transform.GetComponent<Image>().overrideSprite.name) {                                        //判断的是image加载图片的名字                                        
					isFind = true;                                        
		...
		if (isFind == false) {                      
			for (int i = 0; i < cells.Length; i++) {                               
				if (cells [i].transform.childCount == 0) {                                        //当前没有物体,则添加                                      
